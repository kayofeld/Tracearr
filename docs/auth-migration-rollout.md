...
- Reverse proxy: behind an HTTPS reverse proxy, logins work as long as the proxy preserves the Host header, forwards `x-forwarded-host`, or `CORS_ORIGIN` is set to the public URL. Forward `x-forwarded-proto` so session cookies carry the Secure attribute over HTTPS. `TRUST_PROXY` enables client IP resolution from `X-Forwarded-*` headers (recommended behind a proxy so sign-in and Emby-login rate limits count per client instead of per proxy) - set it to the number of proxy hops in front of Tracearr (usually `1`) or a comma-separated list of trusted proxy IPs/CIDRs, not a bare `true`: `true` trusts every hop, so a client can forge its own `X-Forwarded-For` entry and land in a fresh rate-limit bucket per request (security review F3). `true` still works as a discouraged fallback.
...
1. `app/configuration/environment/page.mdx` (canonical env reference)
   ...
   - Add `TRUST_PROXY`: enables client IP and protocol resolution from `X-Forwarded-*` headers. Document the hop-count form first (`TRUST_PROXY=1` for a single reverse proxy) or a comma-separated list of trusted proxy IPs/CIDRs; note that `true` trusts every hop, which lets a client choose the address rate limiting keys on, and is kept only as a discouraged fallback (`apps/server/src/lib/trustProxy.ts`, `apps/server/src/index.ts`).
