...
- Request `status` is an **integer**; observed values `[1, 2, 5]` over all 108. The count
  endpoint implies 1=pending, 2=approved, 3=declined, 4=processing, 5=available/completed.
...
| `status`                                           | unchanged vocabulary `'pending' \| 'approved' \| 'denied' \| 'available'` | Seerr integer mapping at sync: 1→pending, 2→approved, 3→denied, 4→approved (processing = approved-and-fetching), 5→available. Unknown ints → `'pending'` + warn counter (never skip the row; attribution outranks status fidelity). Keeping one vocabulary keeps every status FILTER in the stats SQL unchanged |
