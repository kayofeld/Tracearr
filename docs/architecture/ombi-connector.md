...
| Credentials removed/cleared              | Repeatable job unscheduled (same hook); **data retained** in `ombi_requests`/`ombi_user_mappings` (reconnection keeps history); attribution joins and stats endpoints gate on `configured` and stop surfacing the data — retained but invisible. A purge endpoint is deliberately deferred (§10) |
...
(deferred) purge endpoint, or truncate via DB — acceptable for a single-owner homelab tool,
...
| Item                                                                | Why deferred                                                                                                                                                       |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /ombi/purge` (delete all synced data)                         | Rare need; DB-level truncate acceptable for homelab; add if requested                                                                                              |
